...
data = pdr.get_data_yahoo("GOOGL", start=datetime.date.today(), end=datetime.date.today()+datetime.timedelta(days=1))
...
df = pdr.get_data_yahoo("GOOGL", start=datetime.date.today(), end=datetime.date.today()+datetime.timedelta(days=1)).reset_index()
...
with open(r'GOOGL_stocks.csv','a') as f:
    writer=csv.writer(f)
